...
def overlay_bars(values, image=None, colors=None, scale=10):
    ...
    values = [[int(v * y_scale) for v in val] for val in values]
    ...
    for value, color in zip(values, colors):
        for x,y,o in zip(x_ticks,value, offsets):
            image = cv2.rectangle(image, (x,height - o - y - 1), (x+bar_width, height - o - 1), color, -1)
...
def overlay_entry_points(entry_stats, image):
    ...
    for entry in entry_stats:
        ...
        image = cv2.circle(image, pt, 10, color=color, thickness=-1)
        image = cv2.circle(image, pt, 10, color=(100,200, 121), thickness=1)
