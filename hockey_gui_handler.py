...
class HockeyGuiHandler(object):
    ...
    def _update_image(self):
        ...
        if len(self.entry_statistics) > 0:
            self.entry_histogram = graphics.entry_histogram(self.entry_statistics, self.entry_interval, 60)
            colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]
            graphics_img = graphics.overlay_bars(self.entry_histogram,  image=graphics_img, colors=colors, scale=self.scale)
            image = graphics.overlay_entry_points(self.entry_statistics, image)
